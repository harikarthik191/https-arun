...
import org.app.main.QuestionNAnswerApp;
...
public class UserActivity {
    ...
    public User loginActivity() {
        System.out.println("Welcome to Question & Answer app login !! ");
    ...
    public void createNewUser() {
        ...
        UserRole role = QuestionNAnswerApp.roleFromMenu();
    ...
    public void seeAllQuestions(UserActivity userActivity, User user) throws NumberFormatException, IOException {
        ...
        if (questions.size() == 0) {
            ...
            QuestionNAnswerApp.questionMenu(userActivity, user);//sending the control to question menu
        }
    }
...
//Allows only authorized person to delete the question
        ...
        if (QuestionServiceImpl.questions.size() == 0)
            QuestionNAnswerApp.menu(user, userActivity);//sending control to main menu
    }
    ...
    public void seeAllReplies(UserActivity userActivity, User user) throws NumberFormatException, IOException {
        ...
        Question question = getQuestion();
        ArrayList<Answer> answers = answerService.getAnswers(question);//check this area for bug
        if (answers.size() == 0) {//check this area for bug
            System.out.println("No answer posted yet");
        } else {//check this area for bug
            for (Answer answer : answers) {
                System.out.println(answer.getId() + ". Answer - " + answer.getAnswerBody());
            }
            QuestionNAnswerApp.replyMenu(userActivity, user, question);//sending the control to reply menu
        }
...
//Allows only authorised persons to delete the replies
        ...
        if (answerService.getAnswers(   question).size() == 0)
            QuestionNAnswerApp.questionMenu(userActivity, user);//sending control to question menu
